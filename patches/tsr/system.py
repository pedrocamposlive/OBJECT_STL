...
def _remap_vit_keys(state_dict: dict) -> dict:
    """Remap ViT checkpoint keys from transformers<=4.36 format to 4.37+ format.
    ...
    # Detect whether remapping is needed
    sample = next(iter(state_dict), "")
    if "encoder.layer" not in sample and not any(
        "encoder.layer" in k for k in list(state_dict)[:20]
    ):
    ...
    for old_key, value in state_dict.items():
        ...
        # output.dense (after mlp)  ->  mlp.fc2
        new_key = re.sub(r"\.layers\.(\d+)\.output\.dense\.", r".layers.\1.mlp.fc2.", new_key)
