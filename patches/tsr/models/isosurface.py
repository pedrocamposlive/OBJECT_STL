...
def marching_cubes(level: torch.Tensor, isolevel: float):
    from skimage.measure import marching_cubes as skimage_mc
    verts, faces, _, _ = skimage_mc(
        level.cpu().numpy(), level=isolevel, allow_degenerate=False
    )
    return (
        torch.from_numpy(verts).float(),
        torch.from_numpy(faces.astype(np.int64)).long(),
    )
